...
def launch_flux_jobs(
    ...
    for folder in os.listdir(root_dir):
        ...
        if os.path.isdir(folder_to_use):
            ...
            elif os.path.exists(os.path.join(folder_to_use, "flux_job_loose.inp")) and os.path.exists(os.path.join(folder_to_use, "flux_job_tight.inp")):
                if second_step:
                    print(f"Launching tight job in {folder_to_use}")
                    command_tight = f"cd {folder_to_use} && flux submit flux_job_tight.inp"
                    os.system(command_tight)
                else:
                    print(f"Launching loose job in {folder_to_use}")
                    command_loose = f"cd {folder_to_use} && flux submit flux_job_loose.inp"
