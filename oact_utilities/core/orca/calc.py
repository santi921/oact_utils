...
from oact_utilities.utils.basis import (
    BASIS_DICT,  # noqa: F401  (re-export)
    count_basis_functions,
)
from oact_utilities.utils.create import read_xyz_from_orca

# BASIS_DICT is imported for backwards compatibility with callers that do
# `from ...calc import BASIS_DICT`; it now lives in oact_utilities.utils.basis
# so workflow DB creation can count basis functions without importing this
# module (which pulls in sella and ase.calculators.orca).
...
def get_n_basis(atoms: Atoms) -> int:
    ...
    """
    return count_basis_functions(atoms.get_chemical_symbols())
...
def get_orca_blocks(
    ...
    if simple_input == "omol":
        ...
        if error_handle:
            ...
            if error_code == -1:
                print(
                    "Using looser SCF settings due to previous SCF failure + PModel guess."
                )
