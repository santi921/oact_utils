...
Action flags (at least one required):
  ...
  --include-failed        Also clean scratch from failed/timeout/to_run jobs
  --purge-failed          Purge failed job directories
  --purge-incomplete      FINAL cleanup: purge running/to_run/timeout dirs
                          confirmed incomplete by content (runs --validate-db)
  --validate-db           DB<->folder alignment sanity check (elements + natoms)

Execution control:
  --execute               Actually delete files (default: dry-run)
  --skip-validation       Bypass the --purge-incomplete validation gate (loud)
  --reroot                Resolve jobs by <root_dir>/<basename> (moved corpus)
...
> `--purge-incomplete` / `--validate-db` are for **final-home reclamation of a
> finished corpus**, not an active campaign -- during a live run those statuses
> mean "in flight". See the module docstring in `clean.py` for the full safety
> contract.
...
For long-running Parsl campaigns, `submit_jobs.py` can clean each job's directory the moment its future completes, instead of running `clean.py` afterwards. This prevents scratch files from accumulating during the run and keeps the workflow tree small on Lustre/VAST.
...
Both hooks run after the per-job DB update commits, dispatched from inside the buffered-flush helper so that the purge's TOCTOU re-check (`_purge_failed_job` re-reads `status='failed'` before deleting) is guaranteed to succeed. For crash safety, a light pre-failure marker is written synchronously before the FAILED row is buffered, so a SIGTERM landing during the batch window still leaves a marker on disk. Failures inside the cleanup hooks are logged but never abort the submitter. Traditional mode (`sbatch`/`flux batch`) does not support these flags because the submitter exits before any job finishes; continue to use `clean.py` for that path.
