#!/bin/bash
# =============================================================================
# DRAC single-node Parsl -- BIG lane (61 <= natoms <= 80).
...
#   big    61-80  : 6 x 32  -> ~125 GB/job   <- this file
#   huge   81-100 : 4 x 48  -> ~187 GB/job
#
# BIG rationale: measured avg runtime for 61-80 atoms is ~5.6h with a tail to
# ~14h, so it needs the 24h wall. 6 workers x 32 cores fully packs the node at
# ~125 GB/job -- 4x the budget that OOM-killed a 55-atom job. 32 ranks/job
# trades some ORCA parallel efficiency for zero idle cores; full-pack beats
# running 192 cores half-empty for this large bucket (~3.3k molecules).
# =============================================================================

#SBATCH --account=def-yqw
#SBATCH --time=24:00:00
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=192      # whole node, fully packed: 6 workers * 32 cores
#SBATCH --mem-per-cpu=3900M        # 192 * 3900 = 748,800 MB ~= 731 GB; fits the 768 GB node
#SBATCH --job-name=parsl-drac-big
...
MAX_WORKERS=6
CORES_PER_WORKER=32         # == %pal nprocs; 6*32 = 192 (full pack -> ~125 GB/job)
MIN_ATOMS=61                # band: 61 <= natoms <= 80
MAX_ATOMS=80
BATCH_SIZE=18               # ~3 waves at ~5.6h over a 24h wall. submit_jobs claims ALL of
