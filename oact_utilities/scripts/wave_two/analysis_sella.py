import os
import numpy as np
from oact_utilities.utils.analysis import get_full_info_all_jobs_sella


def save_analysis_data(folder, prefix_save=None, flux_tf=True):
    root_name = folder.split("/")[-2]
    json_data = get_full_info_all_jobs_sella(folder, flux_tf=flux_tf)
    # print(root_name)
    # print(json_data)
    ...
    np.save(
        "./analysis/{}{}.npy".format(root_name, prefix_save if prefix_save else ""),
        json_data,
    )
...
if __name__ == "__main__":
    ...
    root = (
        "/usr/workspace/vargas58/orca_test/maria_benchmarks/wave_2_omol_opt_filtered/"
    )
    for folder in list_of_folders:
        ...
        save_analysis_data(root_directory, flux_tf=flux_tf, prefix_save="omol_sella_")
