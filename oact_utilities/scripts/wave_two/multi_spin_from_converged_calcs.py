...
from oact_utilities.utils.hpc import write_flux_no_template_sella_ase,  write_flux_no_template
from oact_utilities.utils.table_summary import _parse_npy_into_table


actinides_list = ["Ac", "Th", "Pa", "U", "Np", "Pu", "Am", "Cm", "Bk", "Cf", "Es", "Fm", "Md", "No", "Lr"]
actinide_number = [89, 90, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103]
dict_number_to_symbol = {str(num): sym for num, sym in zip(actinide_number, actinides_list)}
...
def read_and_process_data(data_xlsx):
    ...
    list_data = ["CCDC ID", "Actinide", "Formula", "Charge ", "Multiplicity", "Multiplicity_List"]
...
def reopt_for_different_spins(
    ...
    **kwargs
    ...
    table_a_main = _parse_npy_into_table(path_omol, sella=_is_sella_path(path_omol)) if path_omol is not None else None
    table_b_main = _parse_npy_into_table(path_x2c, sella=_is_sella_path(path_x2c)) if path_x2c is not None else None
    table_a_alt = _parse_npy_into_table(path_omol_alt, sella=_is_sella_path(path_omol_alt)) if path_omol_alt is not None else None
    table_b_alt = _parse_npy_into_table(path_x2c_alt, sella=_is_sella_path(path_x2c_alt)) if path_x2c_alt is not None else None
    ...
    for key in sorted(keys):
        ...
        main_a = table_a_main.loc[key].to_dict() if (table_a_main is not None and key in table_a_main.index) else None
        alt_a = table_a_alt.loc[key].to_dict() if (table_a_alt is not None and key in table_a_alt.index) else None
        main_b = table_b_main.loc[key].to_dict() if (table_b_main is not None and key in table_b_main.index) else None
        alt_b = table_b_alt.loc[key].to_dict() if (table_b_alt is not None and key in table_b_alt.index) else None
        ...
        if coords_a is None or coords_b is None:
            print(f"Skipping {key} due to missing coordinates. (omol: {which_a}, x2c: {which_b})")
            missing_list.append(key)
            continue
        ...
        try:
            pos_actindes_a = [np.array(coords_a)[i] for i, elem in enumerate(elems_a) if elem in actinide_number]
            pos_actindes_b = [np.array(coords_b)[i] for i, elem in enumerate(elems_b) if elem in actinide_number]
        ...
        sym_a = elems_to_symbols(elems_a)
        sym_b = elems_to_symbols(elems_b)
        ...
        if data_file_spin_lists is not None:
            ...
            if not row.empty:
                ...
                print(f"Using charge={charge_dummy} and multiplicity={mult_dummy} from data file for {sanitize_key(key)}")
                print(f"Multiplicity list: {mults_to_try}")
            else:
                # skip this molecule if not found in the data file
                print(f"...> No charge/multiplicity found in data file for {sanitize_key(key)}, using defaults.")
                continue
        ...
        for spin in mults_to_try:
            ...
            if spin == mult_dummy:
                print(f"Skipping spin {spin} for {key} (same as original multiplicity).")
                continue
            ...
            try:
                print(f"Writing {'Sella' if tf_sella_omol else 'ORCA'} job for OMOL {key} (spin {spin}) -> {omol_out}")
            ...
            try:
                print(f"Writing {'Sella' if tf_sella_x2c else 'ORCA'} job for X2C {key} (spin {spin}) -> {x2c_out}")
                ...
                x2c_non_actinide_basis = kwargs.get('non_actinide_basis_x2c', 'X2C-TZVPPall')
                x2c_actinide_basis = kwargs.get('actinide_basis_x2c', '/usr/workspace/vargas58/orca_test/basis_sets/cc_pvtz_x2c.bas')
...
def main(): 
    ...
    omol_sella_lot = "/Users/santiagovargas/dev/oact_utils/data/analysis/big/omol_sella/"
    x2c_sella_lot = "/Users/santiagovargas/dev/oact_utils/data/analysis/big/x2c_sella/"
    data_file_spin_lists = "/Users/santiagovargas/dev/oact_utils/data/big_benchmark_filtered/dataset.xlsx"    
    orca_exe = (
    ...
    for cat in list_cats[1:]:
        ...
        # print if test_x2c_sella exists     
