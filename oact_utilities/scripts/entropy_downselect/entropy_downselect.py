...
def load_features(
    features_dir: str, limit: int | None = None,
    ...
    for f, shape in tqdm(
        zip(npy_files, shapes), desc="Loading features", total=len(npy_files),
    ):
        if remaining <= 0:
            break
...
def whiten(
    X: np.ndarray, ref: np.ndarray | None = None, reg: float = 1e-6,
    ...
    if ref is not None:
        debug_log(f"Computing whitening transform from reference ({ref.shape[0]:,} points)...")
    ...
    debug_log(f"Whitening done. Eigenvalue range: [{eigvals.min():.2e}, {eigvals.max():.2e}]")
    return mean, W
...
def compute_all_scores(
    ...
    for i in range(0, N, chunk):
        ...
        chunk_scores = const_term + np.log1p(
            quads.astype(np.float64) / (n_current + 1)
        )
...
def compute_single_score(
    ...
    return float(
        D * np.log(n_current / (n_current + 1))
        + np.log1p(quad / (n_current + 1))
    )
...
def run_selection(
    ...
    while step < n_select:
        ...
        scores = compute_all_scores(
            X, C_inv, mu, n_current, selected, buf_Xc, buf_Q,
        )
        ...
        for _j in range(batch_n):
            ...
            C, C_inv, mu, n_current, dldet = update_state(
                X[best_global], C, C_inv, mu, n_current,
            )
...
def save_outputs(
    ...
    if metadata_frames:
        ...
        selected_meta.to_parquet(
            str(output_dir / "selected_metadata.parquet"), index=False,
        )
...
def build_selected_lmdb(
    ...
    def _resolve(global_idx: int) -> tuple[str, int]:
        pos = np.searchsorted(boundary_starts, global_idx, side="right") - 1
        start, end, stem = boundary_lookup[pos]
    ...
    for rank, gidx in enumerate(tqdm(selected_indices, desc="Mapping indices")):
        stem, local_idx = _resolve(int(gidx))
    ...
    debug_log(f"Writing {n_selected:,} structures to {output_path}...")
    ...
    for chunk_start in tqdm(
        range(0, n_selected, chunk_size), desc="Writing LMDB",
    ):
        chunk_end = min(chunk_start + chunk_size, n_selected)
    ...
    txn = env.begin(write=True)
    txn.put(b"length", pickle.dumps(n_selected, protocol=-1))
    txn.commit()
    env.sync()
    env.close()
    ...
    debug_log(f"Written {n_selected:,} structures + metadata.npz")
    return n_selected
...
if __name__ == "__main__":
    ...
    save_outputs(output_dir, results, args.features_dir, file_boundaries,
