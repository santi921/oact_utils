...
def find_and_launch_flux(root: str, max_depth: int = 5, dry_run: bool = False, verbose: bool = False, skip_done: bool = False, skip_running: bool = False) -> int:
        ...
        if os.path.exists(flux_file):
            ...
            if dry_run:
                ...
                try:
                    ...
                    if skip_running:
                        latest_file = max((os.path.join(d, f) for f in os.listdir(d)), key=os.path.getmtime)
                        latest_mtime = os.path.getmtime(latest_file)
                        if (time.time() - latest_mtime) < 3600:  #
                            print(f"Skipping {d} because it was last modified within the last hour")
                            continue
                    ret = subprocess.run(cmd, shell=True)
                    if ret.returncode == 0:
                        print(f"Launched job in {d}")
                    else:
                        print(f"flux returned non-zero exit code ({ret.returncode}) for {d}")
                    launched += 1
...
def main() -> None:
    ...
    parser.add_argument("--skip-done", action="store_true", help="Skip directories that already have completed")
    parser.add_argument("--skip-running", action="store_true", help="Skip directories that already have a flux job launched")
    args = parser.parse_args()

    if not os.path.isdir(args.root):
        parser.error(f"Root path does not exist or is not a directory: {args.root}")

    n = find_and_launch_flux(args.root, max_depth=args.max_depth, dry_run=args.dry_run, verbose=args.verbose, skip_done=args.skip_done, skip_running=args.skip_running)
